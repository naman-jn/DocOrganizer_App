...
class DatabaseHelper {
  ...
  void _createDb(Database db, int newVersion) async {
    await db.execute(
        'CREATE TABLE $fileTable($colId INTEGER PRIMARY KEY AUTOINCREMENT, $colName TEXT UNIQUE, '
        '$colPath TEXT, $colSize TEXT, $colDate TEXT, $colType TEXT, $colFav INTEGER)');
    await db.execute(
        'CREATE TABLE $tagTable($colIdTag INTEGER PRIMARY KEY AUTOINCREMENT, $colNameTag TEXT UNIQUE, '
        '$colFileIds TEXT)');
  }
...
//		var result = await db.rawQuery('SELECT * FROM $fileTable order by $colSize ASC');
  ...
  Future<int> insertFilesD(List<FileD> files) async {
    print('inserting');
    print('dbInsert length - ${files.length}');
    Database db = await this.database;
    var result;
    files.forEach((file) async {
      result = await db.insert(fileTable, file.toMap());
    });
    return result;
  }
  ...
  Future<int> updateFileD(FileD file) async {
    ...
    var result = await db.update(fileTable, file.toMap(),
        where: '$colId = ?', whereArgs: [file.id]);
    return result;
  }
  ...
  Future<int> deleteFileD(int id) async {
    var db = await this.database;
    int result =
        await db.rawDelete('DELETE FROM $fileTable WHERE $colId = $id');
    return result;
  }
  ...
  Future<int> getCount() async {
    ...
    List<Map<String, dynamic>> x =
        await db.rawQuery('SELECT COUNT (*) from $fileTable');
  ...
  Future<List<FileD>> getFileDList() async {
    ...
    int count =
        fileMapList.length; // Count the number of map entries in db table
...
// Update Operation: Update a Tag object and save it to database
  Future<int> updateTag(Tag tag) async {
    var db = await this.database;
    var result = await db.update(tagTable, tag.toMap(),
        where: '$colIdTag = ?', whereArgs: [tag.id]);
    return result;
  }
...
// Delete Operation: Delete a Tag object from database
  Future<int> deleteTag(int id) async {
    var db = await this.database;
    int result =
        await db.rawDelete('DELETE FROM $tagTable WHERE $colIdTag = $id');
    return result;
  }
...
// Get number of Tag objects in database
  Future<int> getCountTag() async {
    Database db = await this.database;
    List<Map<String, dynamic>> x =
        await db.rawQuery('SELECT COUNT (*) from $tagTable');
    int result = Sqflite.firstIntValue(x);
    return result;
  }
...
// Get the 'Map List' [ List<Map> ] and convert it to 'Tag List' [ List<Tag> ]
  Future<List<Tag>> getTagList() async {
    ...
    int count =
        tagMapList.length; // Count the number of map entries in db table
